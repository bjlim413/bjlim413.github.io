...
- 악의적으로 제작됨
- 자기복사능력 없음
- 유틸리티 내에 내장되거나 위장하여 배포된다

### 7) 스파이웨어
- 사용자 동의 없이 사용자 정보를 빼내어 가는 악성코드

### 8) 애드웨어
- 컴퓨터 사용시 자동적으로 광고가 표시되게 하는 악성코드

### 9) 악성코드
- 컴퓨터에 악영향을 끼칠수 있는 모든 소프트웨어를 총칭
...
### 11) HTTP 1.0와 HTTP 1.1 차이점
- 텍스트와 이미지를 전자는 두 번에 후자는 한 번에 브라우징하는 기술이다

### 12) GET방식
- 클라이언트에서 서버로 보낸 자료가 URL에 노출될 수 있다
- GET방식으로 보내는 자료 양의 한계는 있다

### 13) POST방식
- 입력자료를 URL 뒷부분에 추가해서 보내는 것이 아니라 HTTP 헤더 안에 넣어서 보낸다
...
### 15) PGP
- 필짐머만이 독자적으로 개발한 이메일 보안 프로토콜이다

### 16) PEM
- IETF에서 표준으로 채택된 이메일 보안프로토콜이다
...
### 18) 윈도우의 모드
- 유저모드: RING0, 1,2,3까지 권한을 가지는데 그중 가장 낮은 RING3 권한을 갖게 된다
- 커널모드
...
### 20) 루트킷
- 대표적인 루트킷 중에 lrk5는ㄴ 가장 강력한 루트킷 중에 하나이다
...
### 32) 키로거
- 키보드의 사용자 움직임을 탐지하여 아이디나 패스워드, 계좌번호 등을 몰래 빼가는 공격기법

## 📘3장 - 서버 보안

### 1) 새 계정 생성
- 새 계정 생성시에는 해당 계정 홈디렉토리로 /etc/skel에 존재하는 파일 및 디렉토리가 복사된다
