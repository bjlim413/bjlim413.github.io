---
layout: single
title: 생활 로직
excerpt: 일곱번째 노트
toc: true
toc_label: "추도 예배 순서"
toc_icon: "church"
# sidebar:
  # title: "노트 더보기"
  # nav: sidebar-notes
---

🏡 생활 로직 정리!

# 1. 소개
- 안녕하세요.
- 8년차 소프트웨어 개발자 입니다.
- ERP 개발자로 일하는 중 입니다.

&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 
  
# 2. 경력
> ⚠️ 주식회사STX <br>
> 2020.09 ~ 재직중 (4년 11개월)
> {: .notice--danger}

> ⚠️ 케이씨씨정보통신(주) <br>
> 2017.12 ~ 2020.09 (2년 10개월)
> {: .notice--danger}

## 2.1. 프로그램 운영
<h3>- ERP 프로그램 운영</h3>
  
📓 문의 접수 및 조치방법 안내를 합니다. <br> &emsp;- AP, AR, GL 모듈 관련 이슈사항 처리 <br> &emsp;- TR(예적금, 유가증권, 수금생성, B/L인수) 프로세스 확인 <br> &emsp;- 예산 업로드 실행 및 내역 관련 검증 <br> &emsp;- 명세서, 부가세, Concurrent 검증 및 모니터링 <br> &emsp;- 원가 모니터링 후 미착상품과 상품대체 사후조치 <br> &emsp;- ERP와 I/F된 프로시저 모니터링 및 에러 조치 <br> &emsp;- 출장 전표발행 가이드 및 유지보수 <br> &emsp;- 펌뱅킹/전자세금계산서/EDI 시스템 모니터링 <br> &emsp;- Web Adi 관련 매크로 설정 및 기능 안내 <br> &emsp;- 프로그램 보완개발 및 신규개발 진행 <br> &emsp;- 사용자 계정 생성 및 권한 관리 <br> &emsp;- 데이터 추출 및 전달 <br> &emsp;- 프로세스 안내 및 데이터 의미 설명 
{: .notice--info}

- 2025년 6월

> <h6>예산 업로드 관련 사용자 정보 입력 보완개발</h6>
> 2025.06.25 ~ 2025.06.27
> - 📂
> - 임시 예산 업로드 실행 시 사용자 정보 입력되도록 수정
> - FCM 사용자 검증 후 업로드 가능
> - 업로드 요청 시 계획/실행예산 반영 용이, 예산 업무 편의성 향상
> {: .notice}

> <h6>거래처 인터페이스 관련 주소 중복 보완개발</h6> 
> 2025.06.25 ~ 2025.06.27
> - 📂
> - 등록된 거래처 주소 리스트 중 활성화된 주소만 인식되도록 보완개발
> - 거래처 주소 중복으로 인터페이스 실행 시 에러 발생하여 로직 수정 및 사용자에게 안내
> - 회계팀 사용자가 거래처 화면에서 주소 활성화 여부 선택 가능, 회계 업무 편의성 향상
> {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 

- 2025년 5월
  
> <h6>TMS 전표번호 조회 화면 신규 개발</h6>
> 2025.05.13 ~ 2025.06.05
> - 📂
> - TMS전표를 FCM(오라클)에서 조회 가능하도록 신규 화면 개발
> - 회계전표 리포트, 전표번호 검색, 레코드 음영 및 전표 화면 팝업 기능 제공
> - 회계팀 사용자에게 정확한 전표번호 제공, 회계 업무 처리의 속도 향상
> {: .notice}

- 2025년 1월
  
> <h6>유가증권 평가처리 보완개발</h6>
> 2025.01.02 ~ 2025.01.03
> - 📂
> - 유가증권 평가처리 화면의 외화평가 생성 시 부분매각에 대한 금액 계산 로직 수정
> - 외화평가 생성 시 부분매각에 사용되는 컬럼이 프로세스 상 적절하지 않음을 인지하고 수정 진행
> - 사용자에게 부분매각 금액 계산에 대한 정확한 결과 값 제공, 재무 관리 프로세스의 신뢰성 향상
> {: .notice}

> <h6>해외지사 비용 예산 검증 로직 보완개발</h6>
> 2025.01.02 ~ 2025.01.03
> - 📂
> - 계정별 합계금액 산출 후 가용예산과 비교하는 로직 개발
> - 작성자와 검토자의 각 화면에서 예산 검증시 돌아가는 로직이 상이하여 동일한 로직으로 적용
> - 사용자에게 명확한 예산 초과 경고 메세지를 제공, 예산 관리 프로세스의 신뢰성 향상
> {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 

- 2024년 8월

> <h6> 트롤리고 거래 실적 구분 위한 FCM 보완개발 요청의 건 (추가사항) </h6>
> 2024.08.23 ~ 2024.08.30
> - 📂
> - 운송용역 매출
  - TMS -> GL로 가는 인터페이스 보완개발
> - 운송진행률
  - 원가 -> GL로 가는 인터페이스 보완개발
> - 운송용역 관련 B/L번호가 중복되지 않도록 검증
> - 상품 매출 시점부터 운송진행률, 운송용역까지의 회계/원가 분개처리 프로세스 보완개발(IFRS15)
> {: .notice}
    
- 2024년 7월

> <h6> 트롤리고 거래 실적 구분 위한 FCM 보완개발 </h6>
> 2024.07.02 ~ 2024.07.29
> - 📂
> - TMS에 기재된 트롤리고 거래번호가 인터페이스 실행 후 FCM 전표에 보이도록 보완개발 
> - FCM 전표 생성 → GL 이전 → 계정별원장 조회 가능하도록 프로세스 보완개발
> - 트롤리고 거래건을 구분하여 내역 제공, 원장 관리 편의성 향상
> {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 

- 2023년

> <h6> FCM 시스템 기능 개선 </h6>
> 2023.02 ~ 2023.11
> - 📂
> - 그룹웨어(경조사)-ERP 연동
> - 기획 단계부터 참여, 개발 및 테스트 담당
> - FCM 지급지시서 화면 보완개발
> {: .notice}

- 2022년

> <h6> 그룹웨어, 인사/급여시스템 구축 </h6>
> 2022.02 ~ 2023.07
> - 📂
> - 그룹웨어와 FCM 연동을 위해 시스템 개발 및 소스 수정/보완
> - 구 인사시스템(포스텍)에서 워크업 시스템으로 조직/급여/근태 데이터 이관
> - 출퇴근 기록을 근태 모듈과 연동하여 업무 효율 증대
> {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 

<h3>- 서버 및 OA 운영</h3>

📓 사무자동화로 업무의 편의성을 제공 합니다. <br> &emsp;- 펌뱅킹 서버 운영(환율/법인카드/입금 내역 생성) <br> &emsp;- 전자세금계산서 서버 운영(계산서 발행 및 교부승인) <br> &emsp;- 출입관리시스템 운영(출입카드/지문 등록, 근태 연동) <br> &emsp;- DLP 솔루션 운영(첨부파일승인, 매체제어, URL차단) <br> &emsp;- DRM 솔루션 운영(내부문서보안, 출장라이선스 발급) <br> &emsp;- 웹하드 운영(공유 폴더) <br> &emsp;- 복합기 운영(드라이버 설치 및 사용자 UID 등록) <br> &emsp;- 노트북/모니터 렌탈 및 사원증 발급 <br> &emsp;- 전산소모품 관리 <br> &emsp;- 퇴사자 자료 백업 및 PC 초기화 <br> &emsp;- 화상회의 세팅 및 스케쥴 관리
{: .notice--info}

- 2024년

> <h6>펌뱅킹/전자세금계산서 서버 이관</h6> 
> 2024.06.02 ~ 2024.07.04
> - 📂
> - 신규 서버로 데이터 마이그레이션
> - IP 스위칭 및 서비스 기동
> - 노후화 서버 대체, 시스템 별 이관 방안 수립, 실행
> {: .notice}

- 2023년

> <h6>신규 윈도즈 서버 이관</h6> 
> 2023.02 ~ 2023.03
> - 📂
> - 운영DB로 접속 전환, 관련 DB스크립트 적용, 공인 아이피 스위칭
> - GETMATE시스템(EDI송수신)을 구 서버에서 신규 서버로 이관
> - 서버 공간 및 관리 비용 절약, 시스템 별 이관 방안 수립, 실행
> {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 

## 2.2. 유지보수 관리 
<h3>- IT 계약</h3>

📓 신규/기존 계약 체결을 합니다. <br> &emsp;- EDI 무역 관리 <br> &emsp;- IT 통합 유지보수 <br> &emsp;- 리모트뷰(원격제어)  <br> &emsp;- 리모트콜(원격지원) <br> &emsp;- 그룹웨어 <br> &emsp;- 인사시스템 <br> &emsp;- 장비렌탈 <br> &emsp;- 출입보안 <br> &emsp;- 자사주 정보 이용 <br> &emsp;- 내부회계관리 <br> &emsp;- AWS  <br> &emsp;- 화상회의(ZOOM) <br> &emsp;- 메일 아카이빙 <br> &emsp;- 어도비 제품(동영상편집, 포토샵, 일러스트레이터 등) <br> &emsp;- ChatGPT
{: .notice--info}

- 자사주 정보 이용

> <h6>코스콤</h6> 
> 전자세금계산서 매월 1일 
> - 🚨
> - 계약 : 2020년
> - 갱신 : 계약 종료시 까지 
> - 내용 : 홈페이지에 주식정보 제공
> {: .notice}

- 출입보안

> <h6>케이티텔레캅</h6> 
> 전자세금계산서 매월 5일 
> - 🚨
> - 계약 : 2019년
> - 갱신 : 계약 종료시 까지 
> - 내용 : 출입시스템 서버 및 단말기 관리, 출입보안, 화재이보, 긴급출동
> {: .notice}

- 내부회계관리

> <h6>씨에이에스(CAS)</h6> 
> 전자세금계산서 매월 5일 
> - 🚨
> - 계약 : 2023년
> - 갱신 : 매년 3월
> - 내용 : 내부회계관리 시스템 유지보수
> {: .notice}

- 무역EDI

> <h6>엠투엠글로벌</h6> 
> 전자세금계산서 매월 20일 
> - 🚨
> - 계약 : 2019년
> - 갱신 : 매년 3월
> - 내용 : 무역EDI 송수신 시스템 유지보수
> {: .notice}

- IT 통합 유지보수

> <h6>시큐인프라</h6> 
> 전자세금계산서 매월 25일 
> - 🚨
> - 계약 : 2023년
> - 갱신 : 매년 1월
> - 내용 : IT 인프라 전반에 대한 유지보수, 서버, 네트워크, DB, NMS, M365, 백신, 오피스키퍼
> {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 

<h3>- 거래처</h3>

📓 유지보수를 위해 기술적으로 도와줍니다. <br> &emsp;- 엠투엠글로벌 <br> &emsp;- 시큐인프라 <br> &emsp;- 알서포트  <br> &emsp;- 헬프유 <br> &emsp;- 다우기술 <br> &emsp;- 이수시스템 <br> &emsp;- 한국렌탈 <br> &emsp;- 케이티텔레캅 <br> &emsp;- 코스콤 <br> &emsp;- 씨에이에스 <br> &emsp;- 메가존클라우드 <br> &emsp;- ZOOM <br> &emsp;- 이노탭 <br> &emsp;- Adobe <br> &emsp;- ChatGPT
{: .notice--info}

- 코스콤

> <h6>담당자</h6> 
> 김현준(영업)
> - 📞 
> <a href="tel:01012345678">전화걸기</a>
> {: .notice--warning}
> 김똥똘(세금계산서)
> - 📞 
> <a href="tel:01012345678">전화걸기</a>
> {: .notice--warning}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

## 2.3. 내부통제
<h3>- IT 일반 통제(ITGC)</h3>

📓 감사인 요구사항을 대응 합니다. <br> &emsp;- IT개요 및 환경에 대한 이해 <br> &emsp;- FCM 및 SSO 관련 요청자료 <br> &emsp;- FCM 프로그램 및 데이터 변경 모집단 추출 <br> &emsp;- FCM 및 그룹웨어 계정 권한 부여와 회수 모니터링 <br> &emsp;- 재해복구 훈련 계획 <br> &emsp;- OS 계정 사용 여부 확인 <br> &emsp;- DB 및 서버 시스템 보고서 제출 <br> &emsp;- 그룹웨어 관리자 계정 모니터링 <br> &emsp;- 네트워크 및 서버 구성도 <br> &emsp;- ITGC 설계 및 운영평가 후 개선방안 도출
{: .notice--info}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

## 2.4. SI 프로젝트
<h3>- 한국교육학술정보원 서비스 운영</h3>
- 2020년 2월 ~ 2020년 9월
  
  ```
  근무지: 대구
  개발자: 유지보수 운영관리 경험
  ```
  - ⏰️ 업무
  - ITSM 시스템을 통한 업무협의 및 사용자 요구사항 처리
  - 프로세스 결함에 대한 조치계획(일정,담당자 등)을 수립하여 처리
  {: .notice}

<h3>- 국방부 군수통합프로젝트 계획/예산 개발</h3>
- 2018년 6월 ~ 2020년 2월
  
  ```
  근무지: 용산
  개발자: SI프로젝트 개발 환경 경험
  ```
  - ⏰️ 업무
  - 사업괸리, 중기계획관리, 예산편성관리 담당
  - 군수 데이터 표준화 및 오류 식별 후 코드 재정립
  - 군수 보급품(1~9종) 관련된 예산 프로세스 개발 및 테스트
  {: .notice}
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

<br><br>
# 3. 학력
## 3.1. 대학교
<h3>- 충남대학교</h3>
- 2015년 3월 2일 ~ 2017년 2월 28일
  
  ```
  전공: 경제학
  성적: 3.457/4.5점
  지역: 대전
  ```
  - 🏫 활동
  - 경제연구회 대회 2등 (상금 30만원)
  - 정부3.0 전자정부프레임워크 교육 수강
  {: .notice}
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 

<h3>- California State University, Long Beach</h3>
- 2012년 8월 27일 ~ 2012년 12월 14일
  
  ```
  전공: Economics
  학점: 3.061/4점
  지역: 미국
  ```
  - 🏫 활동
  - Educational Leave for Military Service
  - Tennis Club
  {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

<h3>- El Camino Community College</h3>
- 2010년 7월 5일 ~ 2012년 6월 8일
  
  ```
  전공: Business Economics
  학점: 3.164/4점
  지역: 미국
  ```
  - 🏫 활동
  - Graduated
  - Transfer to California State University, Long Beach
  {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

## 3.2. 고등학교
<h3>- Mary Star of the Sea High School</h3>
- 2008년 8월 25일 ~ 2010년 6월 10일
  
  ```
  성적: 3.4/4점
  지역: 미국
  ```
  - 🏫 활동
  - Graduated
  {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

<h3>- St. Joseph High School</h3>
- 2007년 8월 27일 ~ 2008년 6월 13일
  
  ```
  학점: 3.8/4점
  지역: 미국
  ```
  - 🏫 활동
  - Graduated
  - Transfer to Mary Star of the Sea High School
  {: .notice}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

<h3>- 중앙고등학교</h3>

## 3.3. 중학교
<h3>- 불곡중학교</h3>
<h3>- 상갈중학교</h3>

## 3.4. 초등학교
<h3>- 상갈초등학교</h3>
<h3>- 돌마초등학교</h3>
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

<br><br>
# 4. 스킬
## 4.1. JAVA
<h3>- 문법</h3>
- 형변환

  - 💻
  - ServiceImpl
  - Function 
  {: .notice--warning}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

## 4.2. Oracle DB
<h3>- 문법</h3>
- CRUD

  - 💻
  - Insert
  - Update
  - retrieve
  - delete
  {: .notice--warning}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

## 4.3. Javascript
<h3>- 문법</h3>
- 기본

  - 💻
  - var
  - console.log
  - const
  - function
  {: .notice--warning}

&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>  

<br><br>
# 5. 교육/자격증/기타
## 5.1. 교육
<h3>- 머신러닝, 딥러닝 실무능력 향상과정</h3>
- 2019년 7월 ~ 2019년 8월
  
  ```
  기관: 한국정보보호신업협회
  일정: 매주 토,일 오전9시~오후6시
  ```
  - 🏫 교육내용
  - 머신러닝
  - 딥러닝
  {: .notice}
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br> 

<h3>- 웹해킹방어를 위한 자바시큐어코딩 실무</h3>
- 2018년 3월
  
  ```
  기관: 한국소프트웨어인재개발원
  일정: 3일 (총 24시간)
  ```
  - 🏫 교육내용
  - 시큐어코딩
  - OWASP Top 10 취약점 진단
  {: .notice}
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>

### - 데이터베이스 프로그래밍 초급
- 2018년 2월
  
  ```
  기관: KOSTA
  일정: 3일 (총 24시간)
  ```
  - 🏫 교육내용
  - SQL CRUD
  - SQL JOIN 구문
  {: .notice}
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>

### - 전자정부 표준프레임워크 4.0 통합구현 활용실무 향상과정
- 2018년 1월
  
  ```
  기관: 쌍용강북교육센터
  일정: 5일 (총 40시간)
  ```
  - 🏫 교육내용
  - Spring 프레임워크의 이해
  {: .notice}
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>

### - 자바기반 정보보안 엔지니어
- 2017년 4월 ~ 2017년 9월
  
  ```
  기관: 더조은아카데미학원
  일정: 매주 월~금, 오전9시~오후6시 (총 840시간)
  ```
  - 🏫 교육내용
  - 네트워크 구축 및 보안
  - 시스템 구축 및 보안
  - OWASP Top 10 기반 취약점 진단
  {: .notice}
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>

### - 정부 3.0기반의 JAVA 개발자 양성과정
- 2016년 9월 1일 ~ 2017년 2월 28일
  
  ```
  기관: 한국IT비즈니스진흥협회
  일정:
  - 가을학기 매주 월~금, 오후6시~10시
  - 겨울방학 매주 월~금, 오전9시~오후6시 (총 621시간)
  ```
  - 🏫 교육내용
  - Java(Eclipse), SQL(Oracel DB), HTML, CSS
  - Javascript, jQuery, Spring, Hadoop
  - 준코토미컴퍼니 현장실습(4주)
  {: .notice}
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>

## 5.2. 자격증
### - 정보처리기사
- 2017년 8월 1일
  
  ```
  등록번호: 17202240549A
  발급기관: 한국산업인력공단
  ```
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>

### - 네트워크관리사2급
- 2017년 6월 1일
  
  ```
  등록번호: NT2037461
  발급기관: 한국정보통신자격협회
  ```
  
&emsp;&emsp;<a href="#" class="btn btn--success">Back to top</a>
<br>

## 5.3. 기타

<br><br>
# 6. 외국어


